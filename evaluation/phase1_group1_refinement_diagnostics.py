...
def _first_candidate_snippet(html: str, rule_id: str) -> str:
    ...
    if rule_id == "link-purpose":
        link = soup.find("a")
        if link:
            return str(link)[:320]

    if rule_id in {"aria-required-parent", "aria-required-children", "aria-allowed-role"}:
        role_elem = soup.find(attrs={"role": True})
        if role_elem:
            return str(role_elem)[:320]
...
async def main() -> None:
    ...
    output_path = root / "evaluation" / "phase1_group1_diagnostics_before.json"
    ...
    payload = {
        ...
        "phase": "phase1_group1_refinement",

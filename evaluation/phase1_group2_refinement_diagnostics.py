...
RULE_TO_CHECK = {
    ...
    "landmark-roles": ["landmarks"],
    "button-name": ["buttons"],
}

RULE_EXPECTED_ALIASES = {
    "missing-h1": {"missing-h1", "no-h1", "page-has-heading-one"},
    "multiple-h1": {"multiple-h1"},
    "heading-order": {"heading-order", "heading-skip"},
    "landmark-roles": {"landmark-roles", "no-main-landmark", "landmark-one-main", "landmark-unique"},
...
def _first_candidate_snippet(html: str, rule_id: str) -> str:
    ...
    if rule_id in {"missing-h1", "multiple-h1", "heading-order"}:
        heading = soup.find(["h1", "h2", "h3", "h4", "h5", "h6"])
        if heading:
            return str(heading)[:320]

    if rule_id == "landmark-roles":
        landmark = soup.find(["main", "nav", "header", "footer"])
        if landmark:
            return str(landmark)[:320]

    if rule_id == "button-name":
        btn = soup.find(["button", "input"])
        if btn:
            return str(btn)[:320]
...
async def main() -> None:
    ...
    output_path = root / "evaluation" / "phase1_group2_diagnostics_after.json"
    ...
    payload = {
        ...
        "phase": "phase1_group2_refinement",

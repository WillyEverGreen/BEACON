...
class DOMCrawler:
    ...
    async def _extract_links_from_page(self, page: Any, html: str) -> list[str]:
        ...
        try:
            mutation_links = await page.evaluate("() => window.__beaconMutationLinks || []")
    ...
    async def _inject_mutation_observer(self, page: Any) -> None:
        script = """
            () => {
                if (window.__beaconObserverInstalled) {
                    return;
                }
                window.__beaconObserverInstalled = true;
                window.__beaconMutationLinks = window.__beaconMutationLinks || [];
                const seen = new Set(window.__beaconMutationLinks);
                ...
                const pushLink = (href) => {
                    ...
                    seen.add(href);
                    window.__beaconMutationLinks.push(href);
                };

"""Core crawl queue and URL normalization primitives for Phase 6 site crawling."""
...
class CrawlSession:
    ...
    def dequeue_prioritized(self) -> CrawlQueueEntry | None:
        ...
        best_index = 0
        best_rank = _page_type_rank(self._queue[0].page_type)

        for idx, row in enumerate(self._queue):
            rank = _page_type_rank(row.page_type)
            if rank < best_rank:
                best_rank = rank
                best_index = idx
                if best_rank == 1:

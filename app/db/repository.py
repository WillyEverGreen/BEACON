...
def persist_audit_payload(payload: dict[str, Any], *, status: str = "completed", user_id: str | None = None) -> str:
    ...
    if sb is None:
        logger.info("Supabase unavailable, skipping remote audit persistence for aid=%s", aid)
        return aid
    ...
    try:
        ...
        record_supabase_failure()
        logger.warning("Failed to persist audit payload to Supabase (%s), continuing", exc)
...
def persist_enrichment_payload(
    ...
    *,
    ...
    try:
        ...
        record_supabase_failure()
        logger.warning("persist_enrichment_payload failed (%s)", exc)
...
def get_audit_history(url: str, limit: int = 20) -> list[dict[str, Any]]:
    ...
    try:
        ...
        record_supabase_failure()
        logger.warning("get_audit_history failed (%s)", exc)
        return []
...
def persist_lighthouse_enrichment(scan_id: str, enrichment_block: dict[str, Any]) -> None:
    ...
    try:
        ...
        record_supabase_failure()
        logger.warning("persist_lighthouse_enrichment failed for scan_id=%s: %s", scan_id, exc)

...
def build_scoring_summary(issues: list[dict[str, Any]], *, degraded_mode: bool = False) -> dict[str, Any]:
    ...
    for issue in issues:
        ...
        penalty = severity_score * (5.0 if severity_bucket == "critical" else 3.0 if severity_bucket == "major" else 1.0)
        if severity_bucket == "critical":
            critical_penalty += penalty
        elif severity_bucket == "major":
            major_penalty += penalty
        else:
            minor_penalty += penalty
    ...
    prioritized_issues = sorted(issues, key=_stable_issue_sort_key)

"""
BEACON Fast-Mode Scan — lichess.org (1 page only)
Run: python -m scripts.benchmark_lichess_fast
"""
import asyncio
import json
import os
...
async def main() -> None:
    ...
    print(f"\n{'='*60}")
    print(f"  BEACON Fast-Mode Scan — {TARGET_URL}")
    print(f"{'='*60}\n")
    ...
    if issues_list:
        ...
        for sev in ("critical", "serious", "moderate", "minor", "unknown"):
            ...
            for iss in bucket[:5]:  # Show first 5 per severity
            ...
            if len(bucket) > 5:
                print(f"    ... and {len(bucket) - 5} more")
        print()
    else:
        print("\n  ✅  No issues found by BEACON engines.\n")

    # Dump raw issues as JSON for further analysis
    out_path = os.path.join(os.path.dirname(__file__), "lichess_scan_result_fast.json")

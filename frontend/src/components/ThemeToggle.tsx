...
export function ThemeToggle() {
  ...
  if (!mounted) return <div className="w-8 h-8" />; // Placeholder to avoid CLS

  return (
    <button
      onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
      className="p-2 rounded-md hover:bg-[var(--beacon-surface)] text-[var(--beacon-text-muted)] hover:text-[var(--beacon-text)] transition-colors"
